var World = {
  ...
  init: function(options) {
    ...
    World.LANDMARKS[World.TILE.BEAST_LAIR] = { num: 5, minRadius: 8, maxRadius: World.RADIUS, scene: 'beastLair', label: '妖兽巢穴' };
    World.LANDMARKS[World.TILE.VOID_RIFT] = { num: 3, minRadius: 15, maxRadius: World.RADIUS, scene: 'voidRift', label: '虚空裂隙' };
    ...
    // 小地图与天气
    if (typeof MapSys !== 'undefined') {
      MapSys.createMinimap();
      MapSys.startWeather();
      MapSys.startCaveTimer();
    }
  ...
  useSupplies: function() {
    ...
    if (typeof MapSys !== 'undefined') {
      ...
      var gatherMap = { ';': 'wood', ',': 'meat', '.': 'fur' };
      var rk = gatherMap[terrain];
      if (rk) {
        var bonus = MapSys.getHotspotBonus(World.curPos[0], World.curPos[1], rk);
        if (bonus > 1 && World.foodMove % 2 === 0) {
          Path.outfit[rk] = (Path.outfit[rk] || 0) + (bonus - 1);
          World.updateSupplies();
        }
      }
    }
  ...
  checkFight: function() {
    ...
    if(World.fightMove > World.FIGHT_DELAY) {
      ...
      // 天气影响
      if (typeof MapSys !== 'undefined') {
        chance *= MapSys.getWeatherEffect('fightChance');
      }
      chance *= $SM.hasPerk('stealthy') ? 0.5 : 1;
      if(Math.random() < chance) {
        World.fightMove = 0;
        // 道侣偶遇（天气灵雾时增加概率）
        if (typeof Companion !== 'undefined' && $SM.get('features.location.companion') && Math.random() < 0.25) {
          var terrain = World.getTerrain();
          Companion.rollEncounter(terrain);
        } else {
          Events.triggerFight();
        }
      }
  ...
  doSpace: function() {
    ...
    if(curTile == World.TILE.VILLAGE) {
      ...
      // 洞穴入口检查
      if (typeof MapSys !== 'undefined' && MapSys.hasActiveCave()) {
        var cp = $SM.get('character.mapSys.cavePos', true);
        if (cp && cp[0] === World.curPos[0] && cp[1] === World.curPos[1]) {
          MapSys.enterCave();
          return;
        }
      }
      // 记录探索
      if (typeof MapSys !== 'undefined') MapSys.recordExplore(World.curPos[0], World.curPos[1]);
  ...
  drawMap: function() {
    ...
    map.html(mapString);
    if (typeof MapSys !== 'undefined') MapSys.updateMinimap();
  },
  ...
  goHome: function() {
    ...
    if (World.state.executioner && !$SM.get('features.location.fabricator')) {
      ...
      if (!$SM.get('features.location.secretRealm')) {
        $SM.set('features.location.secretRealm', true);
        Notifications.notify(null, '发现秘境入口！远古修士的试炼之地等待挑战。');
        SecretRealm.init();
      }
  ...
  getMaxHealth: function() {
    ...
    if (typeof Cultivation !== 'undefined') {
      base += Cultivation.getBonusHp();
    }
    if (typeof Technique !== 'undefined') {
      base += Technique.getHpBonus();
    }
    if (typeof Artifact !== 'undefined') {
      base = Math.floor(base * Artifact.getCombatBonus().hpMul);
    }
    return base;
  },
  ...
  getHitChance: function() {
    ...
    if (typeof Cultivation !== 'undefined') {
      base += Cultivation.getBonusHit();
    }
    if (typeof Technique !== 'undefined') {
      base += Technique.getHitBonus();
    }
    if (typeof Artifact !== 'undefined') {
      base += Artifact.getCombatBonus().dodgeChance / 100;
    }
    if (typeof Pet !== 'undefined') {
      base += Pet.getCombatBonus().hitChance / 100;
    }
    if (typeof Companion !== 'undefined') {
      base += Companion.getCombatBonus().dodgeBonus / 100;
    }
