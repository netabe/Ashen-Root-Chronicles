...
Events.Setpieces = {
	...
	"beastLair": {
		...
		scenes: {
			...
			'combat': {
				...
				specials: [],
				onLoad: function() {
					var idx = Math.floor(Math.random() * Beast.Beasts.length);
					var beast = Beast.Beasts[idx];
					Beast._currentBeastIndex = idx;
					var scene = Events.activeEvent().scenes['combat'];
					scene.enemy = beast.name;
					scene.enemyName = beast.name;
					scene.damage = beast.damage;
					scene.hit = beast.hit;
					scene.attackDelay = beast.attackDelay;
					scene.health = beast.hp;
					scene.chara = beast.chara;
					scene.ranged = beast.ranged || false;
					scene.specials = beast.specials || [];
					var loot = {};
			...
			'reward': {
				text: ['击败妖兽！获得战利品。'],
				onLoad: function() {
					var idx = Beast._currentBeastIndex;
					if (idx >= 0) {
						var beast = Beast.Beasts[idx];
	...
	"voidRift": {
		...
		scenes: {
			'start': {
				text: [
					'前方的空间扭曲撕裂，一道幽暗的裂隙在虚空中闪烁。',
					'裂隙中传来令人窒息的压迫感，仿佛连接着另一个世界。'
				],
				onLoad: function() {
					World.markVisited(World.curPos[0], World.curPos[1]);
				},
			...
			'combat': {
				...
				specials: [],
				onLoad: function() {
					var level = Beast.getVoidRiftLevel();
					var scene = Events.activeEvent().scenes['combat'];
					scene.health = Math.min(level * 10 + 50, 300);
					scene.damage = Math.min(level * 3 + 10, 60);
					scene.hit = Math.min(0.6 + level * 0.03, 0.9);
					scene.loot = {
