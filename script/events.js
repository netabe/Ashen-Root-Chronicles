...
var Events = {
		...
		if(Events.activeEvent()) {
			...
			if(Math.random() <= World.getHitChance()) {
				...
				if(typeof dmg == 'number') {
					...
					// 杂灵根天赋灵根觉醒：战斗伤害+20%
					if($SM.hasPerk('mixedRoot3') && typeof dmg == 'number') {
						dmg = Math.floor(dmg * 1.2);
					}
					// 境界加成
					if(typeof dmg == 'number' && typeof Cultivation !== 'undefined') {
						dmg += Cultivation.getBonusDamage();
					}
					// 功法加成
					if(typeof dmg == 'number' && typeof Technique !== 'undefined') {
						dmg += Technique.getDamageBonus();
					}
					// 法宝加成
					if(typeof dmg == 'number' && typeof Artifact !== 'undefined') {
						dmg = Math.floor(dmg * Artifact.getCombatBonus().damageMul);
					}
					// 灵兽加成
					if(typeof dmg == 'number' && typeof Pet !== 'undefined') {
						dmg += Pet.getCombatBonus().damage;
						if(Math.random() * 100 < Pet.getCombatBonus().doubleAtkChance) dmg *= 2;
						if(Math.random() * 100 < Pet.getCombatBonus().critChance) dmg = Math.floor(dmg * 1.5);
					}
					// 道侣加成
					if(typeof dmg == 'number' && typeof Companion !== 'undefined') {
						dmg = Math.floor(dmg * Companion.getCombatBonus().damageMul);
					}
				}
			}
			// 法宝暴击
			if(typeof dmg == 'number' && dmg > 0 && typeof Artifact !== 'undefined') {
				var artBonus = Artifact.getCombatBonus();
				if(artBonus.critChance && Math.random() * 100 < artBonus.critChance) dmg = Math.floor(dmg * 1.5);
			}
	...
	enemyAttack: function() {
		...
		if(!stunned && !meditating) {
			...
			if (typeof Cultivation !== 'undefined') {
				dodge += Cultivation.getBonusDodge();
			}
			if (typeof Technique !== 'undefined') {
				dodge += Technique.getDodgeBonus();
			}
			toHit = Math.max(0, toHit - dodge);
			...
			if (dmg > 0 && typeof Technique !== 'undefined') {
				dmg = Math.max(0, dmg - Technique.getDamageReduction());
			}
			// 法宝减伤
			if (dmg > 0 && typeof Artifact !== 'undefined') {
				dmg = Math.floor(dmg * Artifact.getCombatBonus().armorMul);
			}
			// 灵兽回血
			if (dmg > 0 && typeof Pet !== 'undefined') {
				var petBonus = Pet.getCombatBonus();
				if(petBonus.heal > 0 && Math.random() < 0.3) {
					World.health = Math.min(World.getMaxHealth(), World.health + petBonus.heal);
					$('.hp', '#wanderer').text(World.health + '/' + World.getMaxHealth());
				}
			}
			// 道侣毒伤（对敌人）
			if (dmg > 0 && typeof Companion !== 'undefined') {
				var compBonus = Companion.getCombatBonus();
				if(compBonus.enemyPoison > 0) {
					Events.dotDamage($('#enemy'), compBonus.enemyPoison);
				}
			}
	...
	winFight: function() {
		...
		if (typeof Pet !== 'undefined') {
			var fightsExp = 5 + Math.floor(Math.random() * 10);
			var pets = $SM.get('character.pet.pets', true) || {};
			for (var pi = 0; pi < Pet.MAX_SLOTS; pi++) { if(pets[pi]) Pet.addExp(pi, fightsExp); }
		}
		// 法宝击杀回复
		if (typeof Artifact !== 'undefined') {
			var artB = Artifact.getCombatBonus();
			if (artB.healOnKill > 0) {
				World.health = Math.min(World.getMaxHealth(), World.health + artB.healOnKill);
				var whp = $('.hp', '#wanderer');
				if(whp.length) whp.text(World.health + '/' + World.getMaxHealth());
			}
		}
		// 法宝开场护盾(战斗结束时清理)
		if (typeof Artifact !== 'undefined' && Artifact.getCombatBonus().shield > 0) {
			World.health = Math.min(World.getMaxHealth(), World.health + Artifact.getCombatBonus().shield);
		}
		// 天劫处理
		if (scene && scene.enemy && scene.enemy.isTribulation && typeof Tribulation !== 'undefined') {
			if (scene.onWin && scene.onWin.onLoad) {
				var fn = scene.onWin.onLoad.split('.');
				var obj = window[fn[0]];
				if (obj && obj[fn[1]]) obj[fn[1]]();
				return;
			}
		}
		// 深渊处理
		if (window.Abyss && Abyss._active) {
			if (scene && scene.onWin && scene.onWin.onLoad) {
				var afn = scene.onWin.onLoad.split('.');
				var aobj = window[afn[0]];
				if (aobj && aobj[afn[1]]) aobj[afn[1]]();
				return;
			}
		}
		// 天气加成 - 战利品翻倍
		if (typeof MapSys !== 'undefined' && scene && scene.loot) {
			var wMul = MapSys.getWeatherEffect('lootMul');
			if (wMul > 1) {
				for (var lk in scene.loot) {
					if (scene.loot[lk].min) scene.loot[lk].min *= wMul;
					if (scene.loot[lk].max) scene.loot[lk].max = Math.floor(scene.loot[lk].max * wMul);
				}
			}
		}
		Engine.setTimeout(function() {
