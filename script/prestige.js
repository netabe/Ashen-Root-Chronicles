var Prestige = {
	...
	save: function() {
		...
		// 多周目继承：保存杂灵根天赋
		var perks = $SM.get('character.perks', true);
		var inheritedPerks = {};
		if (perks) {
			for (var i = 1; i <= 5; i++) {
				if (perks['mixedRoot' + i]) {
					inheritedPerks['mixedRoot' + i] = true;
				}
			}
		}
		$SM.set('previous.perks', inheritedPerks);
	},
	...
	collectStores : function() {
		...
		// 多周目继承：恢复杂灵根天赋
		var prevPerks = $SM.get('previous.perks');
		if (prevPerks) {
			for (var k in prevPerks) {
				if (prevPerks[k]) {
					$SM.addPerk(k);
				}
			}
