...
var Room = {
	...
	onFireChange: function () {
		...
		// 杂灵根隐藏天赋：万法初通
		if ($SM.get('game.fire.value') >= 4 && !$SM.hasPerk('mixedRoot1')) {
			$SM.addPerk('mixedRoot1');
			$('body').addClass('cultivation-breakthrough');
			setTimeout(function() { $('body').removeClass('cultivation-breakthrough'); }, 1500);
			Notifications.notify(null, '杂灵根觉醒！灵气浓度达到浓郁，你领悟了"万法初通"。');
		}
	...
	coolFire: function () {
		...
		if ($SM.get('game.fire.value') > 0) {
			// 杂灵根天赋万法初通：灵火更持久
			var coolChance = $SM.hasPerk('mixedRoot1') ? 0.65 : 1;
	...
	buy: function (buyBtn) {
		...
		// 杂灵根天赋五行亲和：交易优惠20%
		if ($SM.hasPerk('mixedRoot2')) {
			var refund = {};
			for (var k in cost) {
				refund[k] = Math.ceil(cost[k] * 0.2);
			}
			$SM.addM('stores', refund);
		}
	...
	build: function (buildBtn) {
		...
		// 杂灵根隐藏天赋触发
		if (thing === 'trading post' && !$SM.hasPerk('mixedRoot2')) {
			$SM.addPerk('mixedRoot2');
			$('body').addClass('cultivation-breakthrough');
			setTimeout(function() { $('body').removeClass('cultivation-breakthrough'); }, 1500);
			Notifications.notify(null, '杂灵根天赋"五行亲和"觉醒！五行灵力流转贯通。');
		}
		if (thing === 'armoury' && !$SM.hasPerk('mixedRoot3')) {
			$SM.addPerk('mixedRoot3');
			$('body').addClass('cultivation-breakthrough');
			setTimeout(function() { $('body').removeClass('cultivation-breakthrough'); }, 1500);
			Notifications.notify(null, '杂灵根天赋"灵根觉醒"爆发！战力大幅提升。');
		}
	},
	...
	handleStateUpdates: function (e) {
		if (e.category == 'stores') {
			...
			Room.updateBuildButtons();
		} else if (e.stateName === 'game.fire.value') {
			Room.checkUnlocks();
		}
	},

	checkUnlocks: function() {
		if (!$SM.get('features.location.cultivation') && $SM.get('game.buildings.hut', true) > 0 && $SM.get('game.fire.value', true) > 0) {
			$SM.set('features.location.cultivation', true);
			$SM.set('features.location.economy', true);
			$SM.set('features.location.pet', true);
			Notifications.notify(Room, '感应到天地灵气，修仙之路开启！');
			Cultivation.init();
			Economy.init();
			Pet.init();
			Engine.updateSlider();
		}
		if (!$SM.get('features.location.alchemy') && $SM.get('game.buildings.workshop', true) > 0) {
			$SM.set('features.location.alchemy', true);
			$SM.set('features.location.companion', true);
			Notifications.notify(Room, '工坊中发现了古老的炼丹玉简！');
			Alchemy.init();
			Technique.init();
			Companion.init();
			if (Engine.activeModule === Room) {
		...
		if (!$SM.get('features.location.tribulation') && Cultivation && Cultivation.getRealmIndex && Cultivation.getRealmIndex() >= 2) {
			$SM.set('features.location.tribulation', true);
			Notifications.notify(Room, '金丹已成，天劫将至！');
			Tribulation.init();
			if (Engine.activeModule === Room) Engine.updateSlider();
		}
		if (!$SM.get('features.location.sect') && Cultivation && Cultivation.getRealmIndex && Cultivation.getRealmIndex() >= 2 && $SM.get('game.buildings["trading post"]', true) > 0) {
			$SM.set('features.location.sect', true);
			Notifications.notify(Room, '声名远播，可开宗立派！');
			Sect.init();
			if (Engine.activeModule === Room) Engine.updateSlider();
		}
		if (!$SM.get('features.location.artifact') && $SM.get('features.location.fabricator')) {
			$SM.set('features.location.artifact', true);
			Notifications.notify(Room, '锻造台传来奇异波动，可炼制法宝！');
			Artifact.init();
			if (Engine.activeModule === Room) Engine.updateSlider();
		}
		if (!$SM.get('features.location.abyss') && Beast && Beast.getBeastKills && Beast.getBeastKills() >= 5) {
			$SM.set('features.location.abyss', true);
			Notifications.notify(Room, '击杀大量妖兽，虚空中浮现万妖窟的入口！');
			Abyss.init();
			if (Engine.activeModule === Room) Engine.updateSlider();
		}
	},
