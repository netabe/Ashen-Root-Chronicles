var Button = {
	Button: function(options) {
		...
		$('<span>').addClass('buttonLabel').text(typeof(options.text) != 'undefined' ? options.text : "button").appendTo(el);

		el.append($("<div>").addClass('cooldown'));
		el.append($("<span>").addClass('cooldownText'));
	...
	cooldown: function(btn, option) {
		...
		if(cd > 0) {
			...
			$('div.cooldown', btn).width((1 - left) * 100 +"%").animate({width: '100%'}, time * 1000, 'linear', function() {
			...
			var cdText = btn.find('.cooldownText');
			cdText.show();
			btn.data('cooldownInterval', setInterval(function() {
	...
	clearCooldown: function(btn, cooldownEnded) {
		...
		if(btn.data('cooldownInterval')){
			clearInterval(btn.data('cooldownInterval'));
			btn.removeData('cooldownInterval');
		}
		btn.find('.cooldownText').hide();
